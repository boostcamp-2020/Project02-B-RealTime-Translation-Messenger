...
## 📌 App Background에서도 끊기지 않는 채팅

### 배경

1️⃣ 모바일 앱의 특성상 채팅 도중 전화가 오거나, 사용자가 다른 화면을 확인하는 등 다양한 상황에서 언제든지 앱은 백그라운드 상태로 진입 가능

2️⃣ 채팅앱의 기본인 **메시지 교환**에 충실하기 위해서, 백그라운드 상태에서도 채팅이 끊어지지 않도록 구현할 필요성

### 적용 방식

➡️ Error를 상황에 맞는 Reconnect로 대체하여 스트림이 끊어지지 않도록 구현

- 소켓 통신에서 error가 발생하여 스트림에 error를 방출한다면, 스트림이 끊어져 이후에 발생하는 이벤트를 받을 수 없음
- 끊어지지 않는 스트림을 유지하기 위하여 error를 방출하는 대신 소켓을 reconnect 하는 방식으로 구현

➡️ App이 Background 혹은 Suspended 상태에서 전송 받은 메시지를 위한 처리

- 화면이 Foreground로 진입 하였을 경우, 마지막 메시지의 타임스탬프를 통해 전송 받지 못한 메시지들을 Request
- 전송 받은 메시지의 타임스탬프를 비교하여 중복된 메시지가 쌓이지 않도록 처리
